...
Será que o algoritmo da listagem \ref{lst:maximo}, para obter o maior elemento de um vetor,  é ótimo? Ou seja, que nenhum outro algoritmo, que utilize como medida de custo o número de comparações consegue encontrar o maior elemento de um vetor $v$ de tamanho $n$ em tempo menor a $f(n) = n - 1$?
...
Normalmente, a medida de custo de execução depende do tamanho da entrada, mas, este não é o único fato que influencia o custo. O custo de execução pode sofrer influência do tipo da entrada. Considere um outro algoritmo para obter o máximo e o mínimo de um vetor de tamanho igual a $n$. Um algoritmo simples para resolver esse problema pode ser derivado do algoritmo \ref{lst:maximo}.
...
O algoritmo \ref{lst:max-min} podem ser facilmente melhorado. Basta observar que a comparação $v[i] < min$ somente é necessária quando o resultado da comparação $v[i] > max$ é falso. Uma nova versão do algoritmo pode ser vista no algoritmo \ref{lst:max-min-2}. Para essa implementação, os casos a considerar são:
...
int[] maximo_minimo_melhorado(int[]  v, int n){
 ...
 A figura \ref{fig:dominacao-assintotica}, expressa graficamente o significado da definição anterior.
 
 \begin{figure}[htpb!]
   \centering
   \includegraphics{01-img/dominacao_assintotica}
...
\begin{example}
   Seja $g(n) = (n + 1) ^ 2$. Logo, $g(n)$ é $O(n^2)$, quando m = 1 e c = 4. Isso porque $(n + 1) ^ 2 \ \leq 4n^2$, para $n \ge 1$. A figura \ref{fig:bigoh} apresenta gráficamente a dominação assintótica que ilustra a notação O.
\end{example}

 \begin{figure}[htpb!]
   \centering
   \includegraphics{01-img/notacao-bigoh}
 ...
 A tabela \ref{tab:operacao-bigoh} apresenta as operações que podem ser realizadas com a notação O.
